...
protected:
  antsDisplacementAndVelocityFieldRegistrationCommandIterationUpdate()
  {
    ...
    this->m_WriteInterationsOutputsInIntervals = 0;    
    this->m_WriteInterationsDisplacementInIntervals = 0;        
...
public:
  ...
  void Execute(const itk::Object * object, const itk::EventObject & event )
  {
    ...
    if( typeid( event ) == typeid( itk::InitializeEvent ) )
      ...
      if( ( this->m_WriteInterationsDisplacementInIntervals != 0 ) &&
        ...
        // The feature can be used to observe the progress of the registration process at each iteration,
        // and make a short movie from the the registration process.
        this->WriteIntervalDisplacement(filter);
        }
  ...
  itkSetMacro( WriteInterationsDisplacementInIntervals, unsigned int );  
  ...
  void WriteIntervalDisplacement(TFilter const * const filter) const
  {
    // //////////////////////////
    // Get output transform from the registration filter at each iteration
    // It can be useful in some cases e.g. when we want to present the registration progress as a series of consequent
    ...
    converter->SetTransform( outputCompositTransform );    
    ...
    */
    if( curIter > 9 )
      {
      currentFileName << "_Iters" << curIter << ".disp.nii.gz";
      }
    else if( curIter > 19 )
      {
      currentFileName << "_Itert" << curIter << ".disp.nii.gz";
      }
    else
      {
...
private:
  ...
  unsigned int m_WriteInterationsDisplacementInIntervals;  
